...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id_abonament'])) {
    ...
    // Calculam datele
    $data_start = date('Y-m-d'); // Azi
    $data_stop = date('Y-m-d', strtotime("+$zile days")); // Azi + 30 zile

    // Dezactivam abonamentele vechi (ca sa nu aiba 2 active)
    $stmt = $pdo->prepare("UPDATE t_abonamente_clienti SET activ = 0 WHERE user_id = ?");
    $stmt->execute([$user_id]);

    // Inseram abonamentul nou
...
// --- VERIFICAM DACA ARE ABONAMENT ACTIV ---
...
// --- CITIM LISTA DE ABONAMENTE DISPONIBILE ---
...
<body>
    ...
    <div class="pricing-container">
        <?php foreach ($tipuri as $tip): ?>
            <?php $cssClass = ($tip['pret'] > 300) ? 'vip' : ''; // Daca e scump, e VIP ?>
